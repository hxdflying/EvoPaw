...
>
> 本文档规划的 `main_crew` / `skill_crew` 指标埋点属于迁移前架构。
> 当前实现已迁移至 Claude Agent SDK，指标埋点的载体已不同。仅作历史参考，**不代表当前可观测性实现**。
>
...
- 使用 Python 标准库 `logging`，在 `main.py` 中统一调用 `setup_logging()` 初始化：
  ...
  - **文件**：结构化 JSON 行日志，写入 `data/logs/evopaw.log`，便于分析与收集
- 日志文件位置：`data/logs/evopaw.log`（滚动日志，单文件 50MB，保留 5 个）
...
**FeishuListener**（`evopaw.feishu.listener`）：
...
| 指标名 | 类型 | Labels | 埋点位置 |
|-------|------|--------|---------|
| `evopaw_feishu_events_total` | Counter | `event_type`, `chat_type` | FeishuListener 解析事件时 |
| `evopaw_inbound_messages_total` | Counter | `routing_key_type`, `has_attachment` | Runner.dispatch 收到 InboundMessage 时 |

**2）HTTP API**

| 指标名 | 类型 | Labels | 埋点位置 |
|-------|------|--------|---------|
| `evopaw_http_requests_total` | Counter | `path`, `method`, `status_code` | TestAPI 请求处理逻辑 |
| `evopaw_http_request_duration_seconds` | Histogram | `path`, `method` | time.monotonic() 测量请求耗时 |

**3）Session 与 Runner 并发**

| 指标名 | 类型 | Labels | 埋点位置 |
|-------|------|--------|---------|
| `evopaw_sessions_active` | Gauge | `routing_key_type` | SessionManager.create_new_session / 清理逻辑 |
| `evopaw_runner_workers_active` | Gauge | `routing_key_type` | Runner._worker 创建/退出时 inc()/dec() |
| `evopaw_runner_queue_size` | Gauge | `routing_key_type` | dispatch / _worker 中根据 queue.qsize() 更新 |

**4）Agent / Sub-Crew（预留）**

| 指标名 | 类型 | Labels | 埋点位置 |
|-------|------|--------|---------|
| `evopaw_agent_requests_total` | Counter | `kind`（main/sub_crew）, `skill_name` | main_crew / skill_crew 启动前 |
| `evopaw_agent_duration_seconds` | Histogram | `kind`, `skill_name` | Agent 执行结束后记录耗时 |
| `evopaw_sub_crews_running` | Gauge | `skill_name` | Sub-Crew 启动/结束时 inc()/dec() |

**5）错误与异常**

| 指标名 | 类型 | Labels | 说明 |
|-------|------|--------|------|
| `evopaw_errors_total` | Counter | `component`, `error_type` | 各模块捕获异常时 inc()，与日志中 error_type 对应 |
