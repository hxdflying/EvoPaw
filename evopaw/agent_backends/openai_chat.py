"""OpenAIChatBackend —— P3 首个真正非 SDK 的 backend。
...
def _build_request_body(
    ...
    # P2-1：通用 generation 参数走 TurnRequest first-class 字段（None 表示不下发）。
            ...
            async with httpx.AsyncClient(timeout=httpx.Timeout(req.timeout_s)) as client:
                for _turn in range(max(1, req.max_turns)):
                    ...
                    if raw_tool_calls and finish_reason in ("tool_calls", None):
                        # P1-4：本轮命中 tool_calls，记 continue。
                    ...
                    # P2-5：循环耗尽未 break，说明每轮都在 tool_calls 上递归。
                    # 与「provider 返回空回复」分开归类，便于主 Agent 给出针对性提示。
