...
class CleanupService:
    ...
    def __init__(
        ...
        self._workspace_dir = (workspace_dir or (data_dir / "workspace")).resolve()
    ...
    def ensure_workspace_dirs(self, session_id: str) -> None:
        ...
        session_base = self._workspace_dir / "sessions" / session_id
    ...
    def write_tavily_credentials(self, api_key: str) -> None:
        """将 Tavily API Key 写入 .config/tavily.json（凭证不经过 LLM）。
        ...
        creds_file = config_dir / "tavily.json"
        ...
        os.replace(tmp_file, creds_file)
        creds_file.chmod(0o600)
        logger.info("cleanup: tavily credentials written to %s (mode 0600)", creds_file)
